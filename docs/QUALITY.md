...
SP-003 establishes isolated schema/migration tests. SP-004 extends authorization/API coverage. SP-005 adds controlled-import coverage. SP-006 adds configuration/auth-gate/secure-session unit and widget regressions plus Android/Windows plugin build evidence. Real owner sign-in and real persisted-session restoration still require an actual hosted environment and target-platform acceptance; mocks/builds cannot prove those boundaries. Camera/reader acceptance and production release configuration remain deferred.
