...
- Authorization: verify server denial for anonymous and unauthorized accounts as well as authorized owner access; hiding buttons is insufficient evidence. Client auth tests must also prove missing configuration and signed-out/expired auth states cannot render protected UI, sign-in is single-flight, and sign-out hides protected UI before remote completion.
...
SP-003 establishes isolated schema/migration tests. SP-004 extends authorization/API coverage. SP-005 adds controlled-import coverage. SP-006 adds configuration/auth-gate/secure-session coverage and platform build evidence. SP-007 adds RPC mapping, bounded search/detail state, stale-response, empty/error, and responsive Arabic rendering regressions. SP-008 adds idempotent-create backend tests plus form validation, confirmed/uncertain mutation reconciliation, unsaved-navigation, and explicit revision-conflict UI regressions. Real server authorization remains supported by the hosted SP-004/SP-006 evidence; mocked catalog repositories do not replace that evidence. Android camera acceptance is recorded for SP-013; Windows reader behavior remains deferred with SP-014. SP-015 establishes release-mode verification and the external production-signing boundary, while real signing credentials/certificates and public distribution remain owner-controlled.
