...
Authentication, catalog search/detail, product create/edit, persistent product drafts, and manual order calculation are implemented. SP-011 adds durable account-scoped local order/page restoration without cloud order sync; automatic refresh and scanner tasks remain. Inventory, sales history, offline catalog replication, licensing, and a separate administration app are deferred.
